...
// Fonction pour créer les services via l'API
async function createServices() {
  for (const service of services) {
    try {
      const response = await fetch('http://localhost:7777/api/service-offerings', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(`❌ Erreur création ${service.name}:`, response.status);
      }
    } catch (error) {
